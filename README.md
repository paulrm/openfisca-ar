...
within the `openfisca_ar` folder.
...
- This country package comes also with *reforms* in the `reforms` folder. This
  ...
  - In this country, there is
    [a reform project](./openfisca_ar/reforms/modify_social_security_taxation.py)
...
The files that are outside from the `openfisca_ar` folder are
...
```sh
git clone git://github.com/paulrm/openfisca-ar.git
cd openfisca-ar
...
```sh
openfisca serve --port 5000 --country-package openfisca_ar
```
...
curl -X POST -H "Content-Type: application/json" \
  -d @./openfisca_ar/situation_examples/couple.json \
